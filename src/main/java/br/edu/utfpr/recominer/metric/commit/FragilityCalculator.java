...
public class FragilityCalculator {
    ...
    public double calcule(final Set<FilePair> files) {
        int pathSum = 0;
        for (FilePair auxFileFile : files) {
            List<String> edges = shortestPath.getPath(auxFileFile.getFile1().getFileName(), auxFileFile.getFile2().getFileName());
            pathSum += edges.size();
        }
