...
public class IssueMetricCalculator {
    ...
    public IssuesMetrics calculeIssueMetrics(final Project project, final Issue issue, final Commit commit) {
        ...
        final String selectMetric = "SELECT DISTINCT im.commit_id, im.id, i.id AS issue_id, iej.issue_key, "
                ...
                + "                  LEFT JOIN {0}.issues_metrics im ON im.issue_id = i.id AND im.commit_id = :commit"
                ...
                + "                  JOIN {0}.issues_scmlog i2s ON i2s.issue_id = i.id AND i2s.scmlog_id = :commit"
        ...
        parameters.addValue("commit", commit.getId());
        parameters.addValue("issue", issue.getId());
        
        IssuesMetrics metrics = namedTemplate.queryForObject(
                QueryUtils.getQueryForDatabase(selectMetric,  project), parameters,
